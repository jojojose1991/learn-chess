...
**27 of 29 resolved. One ticket is actionable right now: 16.**
<!-- /tracker:count -->
...
| Save only checks a Goal it can search: `SEARCH_CEILING` is 2 against a `GOAL_N_MAX` of 10, so a mate in 3–10 is taken on the Coach's word and an unreachable one still stores silently                               | a Puzzle set deeper than 2, or a search cheap enough to raise the ceiling — which also has to handle threefold repetition, path-dependent and absent from a FEN | 29, `src/lib/chess/goals.ts`                                           |
